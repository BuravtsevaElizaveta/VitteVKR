...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False):
    ...
    """
    # Реализуйте NMS или используйте готовую реализацию, например, из библиотеки torchvision
    import torchvision
    ...
    for det in prediction:
        if det is not None and len(det):
            ...
            boxes = det[:, :4]
            scores = det[:, 4]
            classes_scores = det[:, 5:]

            # Определить класс с максимальным баллом
            class_ids = torch.argmax(classes_scores, dim=1)
            class_scores = classes_scores[range(len(classes_scores)), class_ids]

            # Итоговые баллы = confidence * class_score
            final_scores = scores * class_scores

            # Применить NMS
            keep = torchvision.ops.nms(boxes, final_scores, iou_thres)
