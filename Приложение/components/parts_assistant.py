...
import os, io, json, base64, datetime as dt, logging
...
def _proxy_classify(client: OpenAI, img_bytes: bytes, labels: List[str], model: str) -> List[Tuple[str, float]]:
    ...
    sys_msg = ("Ты — CV-ассистент. По фото автозапчасти верни JSON строго формата:\n"
               "{\"top\": [{\"label\": <строка из списка>, \"score\": <число 0..1>}, ...]}")
    user = [{"type":"input_text","text":"Варианты (выбери строго из них): " + ", ".join(labels)},
            {"type":"input_image","image_url":uri}]
    try:
        resp = client.responses.create(model=model, input=[{"role":"system","content":sys_msg},{"role":"user","content":user}],
        ...
        if data and isinstance(data.get("top"), list) and data["top"]:
            pairs = []
            for it in data["top"]:
                lab = str(it.get("label","")).strip(); sc = float(it.get("score",0.0))
                if lab in labels: pairs.append((lab, sc))
            if pairs:
...
def _proxy_translate(client: OpenAI, labels_en: List[str], model: str) -> Dict[str, str]:
    ...
    if not pending: return res
    sys_msg = ("Ты переводчик технических терминов автозапчастей. Верни JSON строго формата:\n"
               "{\"map\": {\"english\": \"русский\"}}")
    user = [{"type":"input_text","text":"Переведи: " + ", ".join(pending)}]
    try:
        r = client.responses.create(model=model, input=[{"role":"system","content":sys_msg},{"role":"user","content":user}],
                                    temperature=0.0, max_output_tokens=200)
        data = _safe_json(_extract_text(r)) or {}
        if isinstance(data.get("map"), dict):
            for k,v in data["map"].items():
                if k in pending and v: res[k] = str(v).strip()
    except Exception as e:
        logging.error(f"[Proxy translate] {e}")
    return res

def _yolo_crops(img_bytes: bytes, weights_path: Path, conf: float = 0.25, iou: float = 0.5, max_crops: int = 8) -> List[bytes]:
    if not _YOLO_OK or not Path(weights_path).exists(): return []
    ...
    for r in res:
        ...
        for b in r.boxes.xyxy.cpu().numpy().astype(int)[:max_crops]:
            ...
            buf = io.BytesIO(); crop.save(buf, format="JPEG", quality=90)
            crops.append(buf.getvalue())
    return crops

def render_parts_assistant(db_path: Path, default_label_map_path: Path|None=None, default_price_map_path: Path|None=None):
    ...
    with st.sidebar:
        ...
        if default_label_map_path and default_label_map_path.exists():
            try:
                ...
                if isinstance(lm, dict): labels = [v for _,v in sorted(((int(k),str(v)) for k,v in lm.items()), key=lambda x:x[0])]
                elif isinstance(lm, list): labels = [str(x) for x in lm]
            except Exception: pass
        labels = [ln.strip() for ln in st.text_area("Список меток (по одной в строке)", value="\n".join(labels), height=220).splitlines() if ln.strip()]
    ...
    with c2:
        ...
        run_btn = st.button("🔍 Определить запчасть", type="primary", use_container_width=True, disabled=not (file or demo_btn))
    img_bytes: Optional[bytes] = None; filename=None
    if file: img_bytes = file.read(); filename=file.name
    elif demo_btn:
        demo_path = Path("ex2.jpg") if Path("ex2.jpg").exists() else Path("ex.png")
        if demo_path.exists(): img_bytes = demo_path.read_bytes(); filename=demo_path.name
        else: st.warning("Демо-файл не найден (ex2.jpg / ex.png)")
    if img_bytes:
    ...
    if run_btn and img_bytes:
        ...
        if not crops: crops = [img_bytes]
        results_all: List[Dict[str, Any]] = []; prog = st.progress(0)
        for i, crop in enumerate(crops):
            pairs = _proxy_classify(client, crop, labels, model)
            labs = [lab for lab,_ in pairs]; ru_map = _proxy_translate(client, labs, model)
            top_en, top_sc = pairs[0]; price = lookup_price(top_en, price_map) or fallback_price(top_en)
            results_all.append(dict(top=pairs, ru_map=ru_map, price=price, crop_index=i))
        ...
        for tab, res in zip(tabs, results_all):
            with tab:
                if len(crops) > 1: st.image(crops[res["crop_index"]], caption=f"crop #{res['crop_index']+1}", width=360)
                df = pd.DataFrame([{"rank":j+1,"label_en":lab,"label_ru":res["ru_map"].get(lab,lab),"score":sc} for j,(lab,sc) in enumerate(res["top"])])
                st.dataframe(df, use_container_width=True)
                price = res["price"]
                st.markdown(f"**Цена (ориентир):** {price['price_min']}–{price['price_max']} {price['currency']} · "
                            f"**Товар:** {res['ru_map'].get(res['top'][0][0], res['top'][0][0])}")
        best = results_all[0]; top_en, top_sc = best["top"][0]; top_ru = best["ru_map"].get(top_en, top_en); price = best["price"]
        insert_detection(
            db_path=db_path, ts=dt.datetime.now().isoformat(timespec="seconds"), kind="parts", source="image",
            filename=filename or "uploaded.jpg", model=model, label_en=top_en, label_ru=top_ru, score=float(top_sc),
            json_top5=json.dumps([{"label_en":lab,"label_ru":best["ru_map"].get(lab,lab),"score":sc} for lab,sc in best["top"]], ensure_ascii=False),
            price_min=float(price["price_min"]), price_max=float(price["price_max"]), currency=price["currency"], image_path=None,
